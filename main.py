...
mqttClient = MQTTClient.Client(
    ...
    f"BLE proxy-{''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=4))}",
)
...
for devAddr, device in config["devices"].items():
    ...
    for service in device["services"].values():
        ...
        for characteristics in service["characteristics"].values():
            devicesData[device["name"]][service["name"]][characteristics["name"]] = {
                "isUpdated": False,
                "isChanged": False,
                "data": None,
            }
...
def parse_inp(data, values, modifier=None, funcToRunOnReplace=None):
    if isinstance(data, str):
        ...
        return {
            parse_inp(key, values, modifier, funcToRunOnReplace): parse_inp(
                value, values, modifier, funcToRunOnReplace
            )
...
async def bleTask(devAddr, deviceConfig: dict):
    ...
    def connectCallback():
        ...
        if "connect" not in deviceConfig["notifyOnChangeState"].keys():
            return
        if not (
            "notifyOnFirstConnect" in deviceConfig["notifyOnChangeState"].keys()
            and deviceConfig["notifyOnChangeState"]["notifyOnFirstConnect"]
        ):  # if do not notify on first connect
    ...
    bleClient = BleakClient(
        devAddr, disconnectCallback, winrt=dict(use_cached_services=False)
    )
    ...
    def loadChar(charConfig, value):
        try:
            if charConfig["loadAs"] == "int":
                ...
                loaded = value.decode(
                    "encoding" in charConfig.keys()
                    if charConfig["encoding"]
                    else "utf-8"
                )  # requires tesing
            ...
            print(
                f"Failed to read characteristic {charConfig['name']} from service {configuredService['name']} in device {deviceConfig['name']} at address {devAddr}: {e}\n maybe bad format of loaded varible"
            )
            return None
        try:
            if "format" in charConfig.keys():
                ...
                formated = loaded
            dataStored = devicesData[deviceConfig["name"]][configuredService["name"]][
                charConfig["name"]
            ]
            ...
            print(
                f"Failed to evaluete characteristic {charConfig['name']} from service {configuredService['name']} in device {deviceConfig['name']} at address {devAddr}: {e}\n check lambda expression"
            )
            return None
        ...
        while not devData["connected"]:
            try:
                ...
                for configuredServiceUuid in configuredServices.keys():
                    ...
                    if service is None:
                        print(
                            f"Service with UUID {configuredServiceUuid} not found for device {deviceConfig['name']} at address {devAddr}"
                        )
                        continue
                    configuredService = configuredServices[configuredServiceUuid]

                    for uuid, charConfig in configuredService[
                        "characteristics"
                    ].items():
                        ...
                        if char is None:
                            ...
                            print(
                                f"Characteristic with UUID {charConfig['name']} not found for service {configuredService['name']} in device {deviceConfig['name']} at address {devAddr}"
                            )
                            continue
                        if (
                            "subscribe" in charConfig.keys()
                            and charConfig["subscribe"] == True
                        ):
                            if "notify" in char.properties:
                                ...
                                try:
                                    ...
                                    print(
                                        f"Failed to set notify for characteristic {charConfig['name']} in service {configuredService['name']} for device {deviceConfig['name']} at address {devAddr}"
                                    )
                devData["connected"] = bleClient.is_connected
                connectCallback()
                break
        ...
        for configuredServiceUuid in configuredServices.keys():
            ...
            for uuid, charConfig in configuredService["characteristics"].items():
                ...
                if char is None:
                    # nonExisteant charracteristics,
                    print(
                        f"Characteristic with UUID {uuid} not found for service {configuredService['name']} in device {deviceConfig['name']} at address {devAddr}"
                    )
                    continue
    ...
    while 1:
        ...
        if doSend:
            ...
            asyncio.run_coroutine_threadsafe(
                mqttSendTask(topicName, topicMessageForm["payload"]),
...
async def main():
    for devAddr, device in config["devices"].items():
        asyncio.run_coroutine_threadsafe(
            bleTask(devAddr, device), asyncio.get_running_loop()
        )

    if "toSendTo" in config["mqtt"]["topics"].keys():
        for topicName, topicMessageForm in config["mqtt"]["topics"]["toSendTo"].items():
            asyncio.run_coroutine_threadsafe(
                mqttCheckTask(topicName, topicMessageForm), asyncio.get_running_loop()
            )
